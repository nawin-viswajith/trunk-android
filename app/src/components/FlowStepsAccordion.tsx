...
/** Every intermediate agent's output collapsed behind a tap, so a multi-step
 * flow reads as a short list of names instead of a wall of text — the
 * user's own final answer (rendered separately by the caller, e.g. the
 * chat bubble or "FINAL RESULT" card) is what's meant to be seen by
 * default; these are the "show your work" steps behind it. */
        ...
        return (
          <View key={step.id} style={styles.row}>
            <Pressable onPress={() => setExpandedId(expanded ? null : step.id)} style={styles.header}>
              ...
              <Text style={styles.meta}>{step.tokensPerSecond.toFixed(1)} tok/s</Text>
