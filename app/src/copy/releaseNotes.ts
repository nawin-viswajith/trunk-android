...
export const RELEASE_NOTES: ReleaseNote[] = [
  { version: "1.3.7b", summary: "Math rendering switched from a WebView (which rendered some formulas as blank boxes on-device) to a direct-to-SVG renderer, with a Show raw/rendered toggle as a fallback. Fixed Mermaid flowcharts not rendering when a node label wrapped across multiple lines. Flowcharts now have a copy button that copies the diagram as an image." },
